/**
 * API endpoint: GET /api/narratives/:id/responses
 * Get all responses that belong to a specific narrative
 */

import type { VercelRequest, VercelResponse } from '@vercel/node';
import { supabase } from '../../_lib/supabase';
import { jsonResponse, handleError, errorResponse } from '../../_lib/response';
import { NarrativeResponsesSchema, safeParseQuery } from '../../_lib/validation';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const parsed = safeParseQuery(NarrativeResponsesSchema, req.query);
    ...
    const { data, error } = await supabase
      ...
      .eq('responds_to_narrative_id', id)
      .order('turn_number', { ascending: true });

    if (error) {
      console.error('[GET /api/narratives/:id/responses] Database error:', error);
